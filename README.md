...
- Red/orange circular target tracking through `/vision_state`.
- Perspective-ellipse target acceptance for tilted target sheets.
- Separate low red rectangular visual obstacle detection on
  `/vision_obstacle_debug`.
- Visual and `/LaserDistance` obstacle avoidance.
- Camera readiness gate: no SEARCH/turning until `/vision_state` is fresh.
- Target-behind-obstacle bypass maneuver with robust reacquire false-positive
  filtering.
- Emergency stop, safe stop scripts, and CSV black-box logging.
...
Place the calibrated orange/terracotta circle in front of the camera. The
target detector accepts both frontal circles and reasonable perspective ellipses
when the target sheet is tilted. The robot centers the object horizontally
(drives `e_x -> 0`) and approaches until the contour area reaches
`target_area_stop`, then holds `GOAL_REACHED`. Remove the object and the robot
enters `SEARCH`, rotating slowly until the target returns. When the target
appears during SEARCH, the robot holds `ACQUIRE_TARGET` briefly before TRACKING
so it does not spin past the object.
...
the robot enters `AVOID` and turns before resuming SEARCH or TRACKING. If the
red box appears to block the target, the controller can run a target-behind-
obstacle bypass maneuver and uses extra confirmation during
`POST_AVOID_REACQUIRE` so edge false positives do not restart AVOID loops. The
